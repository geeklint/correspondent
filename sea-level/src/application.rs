...
///   to effectivly ignore identities.
/// * Integers
/// * `String`
///
...
/// need to be registered, but they must be syntactially valid.  TXT values
/// should preferably be ascii text, and as short as possible (under 200
/// bytes).
///
/// Common approaches to signing certificates incldue storing the CA
...
/// demonstrate this approach).  Another alternitive is to have
...
pub trait Application: 'static + Send + Sync {
    ...
    /// Type to return if certificate signging fails.
